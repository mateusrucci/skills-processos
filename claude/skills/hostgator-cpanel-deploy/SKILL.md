...
```bash
git add .cpanel.yml .github/workflows/deploy.yml shared sites
git commit -m "Configure HostGator cPanel deploy"
git push origin main
gh run list --limit 3
gh run watch RUN_ID
```

Verificar publico com cache-buster:

```bash
curl -I "https://lp.soufit.com/fitmoderno/?v=COMMIT"
curl -I "https://lp.soufit.com/utm-forwarder.js?v=COMMIT"
curl -I "https://lp.soufit.com/meta-tracking.js?v=COMMIT"
curl --show-error --fail-with-body "https://lp.soufit.com/meta-capi.php?v=COMMIT"
```
...
- O repositorio esta exposto como publico em vez do pacote publicado.
- Publicar `sites/<pagina>/index.html` diretamente em `/home3/USER/DOMINIO/<pagina>/index.html`.
